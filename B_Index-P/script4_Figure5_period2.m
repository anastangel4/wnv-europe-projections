...
for i = 1:numel(S)
    ...
    %4.5
    f45 = ['2076-2090/proj45/proj45_' num2str(i) '.csv.estimated_indexP.csv'];
    if isfile(f45)
        T = readtable(f45);
    ...
    %8.5
    f85 = ['2076-2090/proj85/proj85_' num2str(i) '.csv.estimated_indexP.csv'];
    if isfile(f85)
        T = readtable(f85);
        fx = ismember(year(T.date),proj_years) & ismember(month(T.date),months_JJA);
        if any(fx)
...
set(hL,'Rotation',90);
title('\Delta I-P_{Q50} for 2076-2090 Scenario 4.5')
...
hL = ylabel(cb,'\Delta I-P_{Q50}');
set(hL,'Rotation',90);
title('\Delta I-P_{Q50} for 2076-2090 Scenario 8.5')
axis([-30 50 30 85]); set(gca,'FontSize',15)

% Δ Days > threshold (RCP4.5)
figure
...
set(hL,'Rotation',90);
title('\Delta days_{I-P>1.5} for 2076-2090 Scenario 4.5')
axis([-30 50 30 85]); set(gca,'FontSize',15)

% Δ Days > threshold (RCP8.5)
figure
...
hL = ylabel(cb,'\Delta days_{I-P>1.5}');
set(hL,'Rotation',90);
title('\Delta days_{I-P>1.5} for 2076-2090 Scenario 8.5')
axis([-30 50 30 85]); set(gca,'FontSize',15)
